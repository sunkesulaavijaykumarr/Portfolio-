...
const Skills = () => {
  ...
  return (
    <section className="py-12 md:py-20 bg-slate-900" id="skills">
      <div className="container mx-auto px-4 md:px-6">
        <div className="max-w-3xl mx-auto text-center mb-8 md:mb-12">
          <h2 className="text-2xl md:text-4xl font-bold mb-3 md:mb-4">
            Technical <span className="text-blue-500">Skills</span>
          </h2>
          <div className="h-1 w-20 bg-blue-500 mx-auto rounded-full mb-6 md:mb-8"></div>
        ...
        <div className="flex flex-wrap justify-center gap-2 md:gap-4 mb-8 md:mb-12">
          {categories.map((category) => (
            <button
              ...
              className={`px-3 md:px-4 py-1.5 md:py-2 rounded-lg text-xs md:text-base flex items-center gap-1.5 md:gap-2 transition-all ${
                activeCategory === category.title
                  ? 'bg-blue-600 text-white shadow-lg'
                  : 'bg-slate-800 text-slate-300 hover:bg-slate-700'
              }`}
            >
              {React.cloneElement(category.icon as React.ReactElement, { size: isMobile ? 16 : 20 })}
              {category.title}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 max-w-6xl mx-auto">
          {activeSkills.map((skill, index) => (
            <div
              ...
              className="bg-slate-800/50 p-4 md:p-6 rounded-xl border border-slate-700 hover:border-blue-500/50 transition-all duration-300"
            >
              <div className="flex items-start gap-3 md:gap-4">
                <div className="w-10 h-10 md:w-12 md:h-12 rounded-lg bg-slate-700/50 p-2 md:p-2.5 flex items-center justify-center">
                  {skill.logo ? (
                    ...
                    React.cloneElement(skill.icon as React.ReactElement, { size: isMobile ? 16 : 20 })
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="text-base md:text-lg font-semibold mb-2 truncate">{skill.name}</h3>
                  <div className="w-full bg-slate-700 rounded-full h-2">
                    <div
                      className={`h-full rounded-full ${skill.color} animate-skillProgress`}
                      style={{
                        width: `${skill.level}%`,
                  ...
                  <div className="flex justify-between mt-1">
                    <span className="text-xs md:text-sm text-slate-400 truncate">{skill.subCategory}</span>
                    <span className="text-xs md:text-sm text-slate-400 ml-2">{skill.level}%</span>
                  </div>
                </div>
              </div>
            </div>
